...
- (void)viewDidLoad
{
	...
	UIButton *leftButtonImg = [[[UIButton alloc] initWithFrame:CGRectMake(5, 7, 50, 30)] autorelease];
    ...
    {
        leftButtonImg = [[UIButton alloc] initWithFrame:CGRectMake(5, 12, 50, 20)];
        
        [leftButtonImg setImage:[UIImage imageNamed:@"backNew_1.png"] forState:UIControlStateNormal];
        [self.nexButton setImage:[UIImage imageNamed:@"next-red_1.png"] forState:UIControlStateNormal];
        [self.preButton setImage:[UIImage imageNamed:@"back-red_1.png"] forState:UIControlStateNormal];
    }
...
- (void)loadArrayOfCards:(NSArray*)cards withParentViewC:(DeckViewController*) prntViewC
{
	...
	{
		index = tempIndex	+ i;
        CustomWebView* page = [[CustomWebView alloc] initWithFrame:CGRectMake(self.view.frame.size.width * i, 0, self.view.frame.size.width, _scrlView.frame.size.height)];
...
- (IBAction)loadNextCardDetails
{
	...
	if(_selectedCardIndex + 1 < [_arrayOfCards count])
	{
		...
		[_scrlView setContentOffset:CGPointMake(self.view.frame.size.width * _selectedCardIndex, 0) animated:YES];
...
- (void)updateNavBar
{
    ...
    if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"7.0"))
    {
        ...
        if (card.cardID!=0 && [[AppDelegate_iPad getDBAccess] isCommentOrNotesAvailable:card.cardID]) {
            ...
            [actionImg setImage:[UIImage imageNamed:@"exter-green.png"] forState:UIControlStateNormal];
...
-(void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
	...
	if ([title isEqualToString:@"Voice Notes"]) {
		...
		detailViewController.view.frame = CGRectMake(382, 44, self.view.frame.size.width, self.view.frame.size.height);
